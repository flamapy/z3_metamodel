...
class Z3AttributeOptimization(AttributeOptimization):
    ...
    def execute(self, model: VariabilityModel) -> 'Z3AttributeOptimization':
        ...
        self._result = optimize_pareto(z3_model, self._attributes, self.report_progress)
        return self
    
    def execute_solver(self, model: VariabilityModel, solver: z3.Solver) -> 'Z3AttributeOptimization':
        z3_model = cast(Z3Model, model)
        # Validate attributes
        for attr_name in self._attributes:
            if attr_name not in z3_model.attributes_types:
            ...
            if attr_type not in [AttributeType.INTEGER, AttributeType.REAL]:
                raise FlamaException(f'Only numerical attributes (Integer, Real) can be optimized.'
                                     f' Attribute "{attr_name}" is of type {attr_type}.')
        self._result = optimize_pareto(z3_model, self._attributes, self.report_progress, solver)
        return self
...
def optimize_pareto(z3_model: Z3Model,
    ...
    if len(attributes) == 1:
        ...
        return optimize_single_objective(z3_model, attr, goal, report_progress, solver)
    else:
        return optimize_multi_objective(z3_model, attributes, solver)
...
def optimize_multi_objective(z3_model: Z3Model,
    ...
    while solver.check() == z3.sat:
        ...
        if report_progress is not None:
            report_progress(len(pareto_solutions))
    ...
    for i in range(len(pareto_solutions)):
        ...
        if not is_dominated_by_another:
            ...
            if report_progress is not None:
                report_progress(len(final_pareto_front))
